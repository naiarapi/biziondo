<?php
/*Actualiación a la versión 7.0 de PHP por Javier Esparza <jesparza65@gmail.com> (21-10-2016)

Actualización a la versión 5.3 de PHP por www.uterra.com (17-06-2013)

El código original fue desarrollado por Jorge Pinedo Rosas

Buscar actualizaciones del codigo original con el criterio: xxx */
...
die (" Error en la consulta de conteo de registros: $_pagi_sqlConta. Mysql dijo: <b>".mysqli_error($con)."</b>");
}
$_pagi_totalReg = mysqli_data_seek($_pagi_result2,0);	// Total de registros xxx - Se ha agregado @ para evitar que se muestre error cuando los registros son 0
$_Num_regis_Consul = mysqli_num_rows($_pagi_result2); 	// Agregamos esta linea xxx
}else{
$_pagi_result3 = mysqli_query($con,$_pagi_sql);
// Si ocurrió error y mostrar errores está activado
if($_pagi_result3 == false && $_pagi_mostrar_errores == true){
die (" Error en la consulta de conteo alternativo de registros: $_pagi_sql. Mysql dijo: <b>".mysqli_error($con)."</b>");
}
$_pagi_totalReg = mysqli_num_rows($_pagi_result3);
$_Num_regis_Consul = mysqli_num_rows($_pagi_result); 	// Agregamos esta linea xxx
}
...
// Si el número de página es la actual ($_pagi_actual). Se escribe el número, pero sin enlace y en negrita...estilo class aplicado para diferenciarlo!!
$_pagi_navegacion_temporal[] = "<span class='negrita-pag' ".$_pagi_nav_estilo_mod.">$_pagi_i</span>";
}else{
...
die ("Error en la consulta limitada: $_pagi_sqlLim. Mysql dijo: <b>".mysqli_error($con)."</b>");
}
